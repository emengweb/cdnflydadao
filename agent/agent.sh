...
if [[ $MASTER_VER == "" ]]; then
    ...
    agent_ver=`(curl -s -m 5 "http://auth.fikkey.com/master/upgrades?version_num=$version_num" || curl -s -m 5 "https://github.com/LoveesYe/emengweb/cdnflydadao/raw/main/web/upgrades?version_num=$version_num") | grep -Po '"agent_ver":"\d+"' | grep -Po "\d+" || true`
    if [[ "$agent_ver" == "" ]]; then
        echo "无法获取agent版本"
        exit 1
    fi
...
download "https://github.com/LoveesYe/emengweb/cdnflydadao/raw/main/agent/$tar_gz_name" "https://github.com/LoveesYe/emengweb/cdnflydadao/raw/main/agent/$tar_gz_name" "$tar_gz_name"
